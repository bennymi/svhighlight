...
<script lang="ts">
	...
	/** Set the label text. */
	export let label: string = 'Table of Contents';
	...
	let elementToHeading: any = {}; // lookup table to see which heading an element belongs to
	let headingsParents: any = {}; //lookup table to see which parent headings a heading has
	let activeParents: number[] = [];
	let observer: IntersectionObserver | null = null;
	let visibleElementIndexes: number[] = [];
	let observerThreshold: number = 0.25; // used for the Intersection Observer API
